...
EntityBase {
    ...
    Timer {
        id: timer
        running: boss.visible//scene.visible == true && splashFinished // only enable the creation timer, when the gameScene is visible
    ...
    function fireDo2(){
         var imagePointInWorldCoordinates = mapToItem(level,monsterImage.imagePoints[0].x, monsterImage.imagePoints[0].y)
        entityManager.createEntityFromUrlWithProperties(Qt.resolvedUrl("Rocket1.qml"), {"x": imagePointInWorldCoordinates.x-40, "y": imagePointInWorldCoordinates.y, "rotation": boss.rotation,"power":10900})
        entityManager.createEntityFromUrlWithProperties(Qt.resolvedUrl("Rocket1.qml"), {"x": imagePointInWorldCoordinates.x+40, "y": imagePointInWorldCoordinates.y, "rotation": boss.rotation,"power":10900})
    }
